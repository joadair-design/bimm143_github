...
Make sure to remove the first `diagonsis` column - I don’t want to use
...
The main functions here is `prcomp()` and we want to make sure we set
...
- similar characteristics will cluster together - Cell PC1 score (cell1
\# influence on PC1 \#)+() - low weight PCs don’t contribute/influence
the cells much compared to the higher weighted PCs

> Q8. Generate a similar plot for principal components 1 and 3. What do
> you notice about these plots? - I notice that they are more towards
...
Collectively these two plot (“score plot” and “loading plot”) tells us
that if cells nucleus are deeply indented (“conclave”), irregular
...
You can also use the `cutree()` function with a arugment `k=4` rather
than `h=height`
...
Here we will take our PCA results and use those input for clustering. In
...
There is a high specificity and strong sensitivity compared to
...
> based on your results? - We should prioritize the patient with cluster
> 1 for a follow up because they are maglignant.
